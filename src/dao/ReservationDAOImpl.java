...
public class ReservationDAOImpl implements ReservationDAO {
	...
	ReviewDatabase reviewdb = ReviewDatabase.getInstance();
...
//		}
	...
	//예약하려는 룸아이디를 가져오면 이미 reserv된 리스트를 가져온다. 취소되지 않은 녀석이어야 한다.
		public ArrayList<ReservationVO> reservedRoom(int roomid){				// status 1- 예약완료 , 2- 예약취소 , 3 - 리뷰가능, 4 - 리뷰완료
...
//			for(int i = 0; i < reviewdb.tb_review.size(); i++){
//				if(reviewdb.tb_review.get(i).getRoomId() == rvo.getRoomId()){
//					roomid = reviewdb.tb_review.get(i).getRoomId();
//				}
//				if(reviewdb.tb_review.get(i).getUserId() == rvo.getUserId()){
//					userid = reviewdb.tb_review.get(i).getUserId();
//				}
//			}
					
//			for(int i = 0; i < database.tb_reservation.size(); i++){
//				if(database.tb_reservation.get(i).getRoomId() == rvo.getRoomId()){
//					roomid2 = database.tb_reservation.get(i).getRoomId();
//				}
//				if(database.tb_reservation.get(i).getUserId() == rvo.getUserId()){
//					userid2 = database.tb_reservation.get(i).getUserId();
//				}
//			}
			String status = "";
			if(rvo.getStatus()==1){status = "예약완료/취소가능";}
			if(rvo.getStatus()==2){status = "예약취소";}
//			if(rvo.getStatus()==3 && roomid == roomid2 && userid == userid2){
//				status = "리뷰작성완료";
//			}else{
//				status = "리뷰작성가능";
//			}
			if(rvo.getStatus()==3){status = "리뷰작성가능";}
			if(rvo.getStatus()==4){status = "리뷰작성완료";}

			
			DecimalFormat form =new DecimalFormat("###,###");
			
			System.out.println("\t\t\t\t☆ 예약 목록 조회 페이지 입니다 ☆");
			moongu1();
			System.out.println();
